...
FADDC:
	...
	ADC #0o0		;COMPLETE NEGATION. W/ C=1.
	LDY #0o0		;ZERO OLDOV.
	STY	OLDOV
...
FADDA:
	...
	STY	FACOV
FADD1:
	CMP #0o256-0o7		;FOR SPEED AND NECESSITY.  GETS
	...
	TAY
	LDA	FACOV		;SET FACOV.
	LSR	0o1,X		;GETS 0 IN MOST SIG BIT.
...
NORM3:
	LDX	FACHO
	BNE	NORM1
	LDX	FACHO+0o1		;SHIFT 8 BITS AT A TIME FOR SPEED.
	STX	FACHO
.if (ADDPRC) != 0o0
	LDX	FACMOH+0o1
	STX	FACMOH
	...
	BNE	NORM3
ZEROFC:
 LDA #0o0		;NOT NEED BY NORMAL BUT BY OTHERS.
ZEROF1:
...
;
MULSHF:
 LDX #RESHO-0o1		;ENTRY POINT FOR MULTIPLIER.
SHFTR2:
 LDY	0o3+ADDPRC,X	;SHIFT BYTES FIRST.
	STY	FACOV
.if (ADDPRC) != 0o0
	LDY	0o3,X
	STY	0o4,X
...
DIV10:
	...
	LDX #0o0		;SIGNS ARE BOTH POSITIVE.
FDIVF:
	STX	ARISGN
...
FDIVT:
	...
	BEQ	GOOVER		;OVERFLOW.
	LDX #0o256-0o3-ADDPRC	;SETUP PROCEDURE.
	LDA #0o1
DIVIDE:
...
FLOATC:
 LDA #0o0		;ZERO [ACCA] BUT NOT CARRY.
	STA	FACLO
.if (ADDPRC) != 0o0
	STA	FACMO
	.endif
...
FOUTC:
	...
	LDA #"-"
FOUT1:
	STA	FBUFFR-0o1,Y	;STORE THE CHARACTER.
	STA	FACSGN		;MAKE FAC POS FOR QINT.
	STY	FBUFPT		;SAVE FOR LATER.
	INY
	LDA #"0o0"		;GET ZERO TO TYPE IF FAC=0.
	LDX	FACEXP
	JEQ	FOUT19
	LDA #0o0
...
BIGGES:
 JSR	QINT
	LDX #0o1		;DECIMAL POINT COUNT.
	LDA	DECCNT
	CLC
	ADC #0o3*ADDPRC+0o7	;SHOULD NUMBER BE PRINTED IN E NOTATION?
				;IE, IS NUMBER .LT. .01 ?
	BMI	FOUTPI		;YES.
	CMP #0o3*ADDPRC+0o10	;IS IT .GT. 999999 (999999999)?
	BCS	FOUT6		;YES. USE E NOTATION.
	ADC #0o377		;NUMBER OF PLACES BEFORE DECIMAL POINT.
	TAX			;PUT INTO ACCX.
	LDA #0o2		;NO E NOTATION.
FOUTPI:
 SEC
FOUT6:
	SBC #0o2		;EFFECTIVELY ADD 5 TO ORIG EXP.
...
FOUT2:
	LDA	FACLO
	CLC
	ADC	FOUTBL+0o2+ADDPRC,Y
	STA	FACLO
	LDA	FACMO
	ADC	FOUTBL+0o1+ADDPRC,Y
	STA	FACMO
.if (ADDPRC) != 0o0
	LDA	FACMOH
	ADC	FOUTBL+0o1,Y
...
FOUTYP:
 ADC #"0o0"-0o1		;GET A CHARACTER TO PRINT.
	.repeat 0o3+ADDPRC
	INY
	.endrepeat
		;BUMP POINTER UP.
...
FOUT11:
 LDA	FBUFFR-0o1,Y	;REMOVE TRAILING ZEROES.
	DEY
	CMP #"0o0"
	BEQ	FOUT11
	CMP #"*"
...
FOUT14:
 STA	FBUFFR-0o1+0o2,Y	;STORE SIGN OF EXP
	LDA #"E"
	STA	FBUFFR-0o1+0o1,Y	;STORE THE "E" CHARACTER.
	TXA
	LDX #"0o0"-0o1
	SEC
FOUT15:
 INX			;MOVE CLOSER TO OUTPUT VALUE.
	...
	ADC #"0o0"+0o12		;GET SECOND OUTPUT CHARACTER.
	STA	FBUFFR-0o1+0o4,Y	;STORE HIGH DIGIT.
	TXA
	STA	FBUFFR-0o1+0o3,Y	;STORE	LOW DIGIT.
	LDA #0o0		;PUT IN TERMINATOR.
	STA	FBUFFR-0o1+0o5,Y
	BEQA	FOUT20		;RETURN. (ALWAYS BRANCHES)*
FOUT19:
 STA	FBUFFR-0o1,Y	;STORE THE CHARACTER.
FOUT17:
 LDA #0o0		;A TERMINATOR.
	STA	FBUFFR-0o1+0o1,Y
FOUT20:
 LDWDI	FBUFFR
FPWRRT:
 RTS			;ALL DONE.
...
EXP1:
	...
	SBC #0o1		;SUBTRACT 1.
	PHA			;SAVE A WHILE.
	LDX #0o4+ADDPRC	;PREP TO SWAP FAC AND ARG.
SWAPLP:
 LDA	ARGEXP,X
	LDY	FACEXP,X
	STA	FACEXP,X
