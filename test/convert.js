        ...
        for (let i = 0; i < line.length; i++) {
            ...
            if (/[A-Za-z_%@]/.test(ch)) {
                ...
                if (current && !keywords.has(current.toUpperCase())) {
                    ...
                    if (!symbolMap.has(key6))
                        symbolMap.set(key6, new Set());
...
function normalizeSymbolsInLine(line) {
    ...
    const isRadixLiteral = (tok) => result.endsWith("^") && /^[ODB][0-9]+$/i.test(tok);
...
export function convertMacro10ToK65(content) {
    ...
    function performReplacements(text, macroArgs) {
        ...
        while (changed && iterations < 10) {
            ...
            if (currentRadix === 8) {
                ...
                current =
                    code.replace(/(?<![\w.$@^\\])[0-7]+\b/g, (d) => parseInt(d, 8) < 8 ? String(parseInt(d, 8)) : `0o${d}`) + rest;
            }
            // Explicit octal `^O...`: a single-digit value (0-7) is the same in
            // decimal, so drop the prefix; larger values keep `0o`.
            current = current.replace(/\^O([0-7]+)/g, (_m, oct) => parseInt(oct, 8) < 8 ? String(parseInt(oct, 8)) : `0o${oct}`);
            ...
            current = current.replace(/^(\s*)(\.?XLIST|\.?XCREF|\.?CREF|PURGE\b|IFNDEF\b|END(?=\s|$))(.*)$/, "$1; $2$3");
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        while (processing) {
            ...
            const currentArgs = lastBlock && lastBlock.type === "macro" ? lastBlock.args : [];
            // 1. Extract Labels (at start of line)
            const labelMatch = line.match(/^(\s*)([\@A-Za-z0-9_\$]+)::?!?(.*)/);
            if (labelMatch) {
                finalizeAndPush((labelMatch[1] ?? "") + (labelMatch[2] ?? "") + ":", currentArgs);
            ...
            let match;
            if ((match = line.match(/^(\s*)DEFINE\s+([A-Za-z0-9_\$]+)(?:\s*\((.*)\))?\s*,?\s*<(.*)/))) {
                const args = (match[3] ?? "")
                    .split(",")
                    .map((s) => s.trim())
                    .filter((s) => s);
                const declaredArgs = args.map(renameParam);
                finalizeAndPush(`${match[1]}.macro ${match[2]}${declaredArgs.length > 0 ? ", " + declaredArgs.join(", ") : ""}`, currentArgs);
                blockStack.push({ type: "macro", args, startDepth: angleDepth });
                angleDepth++;
                line = (match[1] ?? "") + (match[4] ?? "");
                continue;
            }
            else if ((match = line.match(/^(\s*)(IFE|IFN|IF1|IF2)\b\s*(.*)$/)) &&
                (match[3] ?? "").includes("<")) {
                ...
                const cond = performReplacements(condRaw
                    .replace(/</g, "(")
                    .replace(/>/g, ")")
                    .replace(/!/g, "|")
                    .trim(), currentArgs);
