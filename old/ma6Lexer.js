...
export const Comment = createToken({
    name: "Comment",
    pattern: (text, startOffset) => startOffset === 0 ? null : /;.*/.exec(text.substring(startOffset)),
    start_chars_hint: [";"],
    line_breaks: false,
});
...
export const STRING = createToken({
    ...
    pattern: (text, startOffset) => {
        const result = /"([^"\\]|\\.)*"/.exec(text.substring(startOffset));
        if (result && result[1])
            result.payload = unescapeBasic(result[1]);
        return result;
    },
    start_chars_hint: ['"'],
    line_breaks: false,
...
export const HEXNUM = createToken({
    name: "HEXNUM",
    pattern: (text, startOffset) => {
        const result = /(?:0x|\$)([0-9a-fA-F]+)/.exec(text.substring(startOffset));
        if (result && result[1])
            result.payload = parseInt(result[1], 16);
        return result;
    },
    start_chars_hint: ["0", "$"],
    line_breaks: false,
...
export const OCTNUM = createToken({
    name: "OCTNUM",
    pattern: (text, startOffset) => {
        const result = /0o([0-7]+)/.exec(text.substring(startOffset));
        if (result && result[1])
            result.payload = parseInt(result[1], 8);
        return result;
    },
    start_chars_hint: ["0"],
    line_breaks: false,
...
export const BINNUM = createToken({
    name: "BINNUM",
    pattern: (text, startOffset) => {
        const result = /(?:0b|%)([01]+)/.exec(text.substring(startOffset));
        if (result && result[1])
            result.payload = parseInt(result[1], 2);
        return result;
    },
    start_chars_hint: ["%", "0"],
    line_breaks: false,
...
export const DECNUM = createToken({
    name: "DECNUM",
    pattern: (text, startOffset) => {
        const result = /[0-9]+/.exec(text.substring(startOffset));
        if (result)
            result.payload = parseInt(result[0], 10);
        return result;
    },
...
function unescapeBasic(str) {
    ...
    return str.replace(/\\([0abtnvfr\\])/g, (_, char) => map[char]);
}
...
//# sourceMappingURL=ma6Lexer.js.map